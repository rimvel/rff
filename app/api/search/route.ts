    ...
    try {
        if (dateRangeDays) {
            ...
            const allResults = await Promise.all(
                datesToSearch.map(async (searchDate) => {
            ...
            if (returnDate) {
                ...
                const combinedResults = outboundResults.map(outbound => ({
                    ...outbound,
                    returnFlights: validReturnFlights,
                    isRoundTrip: true,
                    totalPrice: outbound.totalPrice + (validReturnFlights[0]?.totalPrice || 0)
                ...
                console.log('Combined results:', combinedResults.length, 'with return flights:', combinedResults[0]?.returnFlights?.length);
                return NextResponse.json(combinedResults);
            }

            return NextResponse.json(outboundResults);
        }
    } catch (e) {
